...
function areaCoordsMain(imode) {
    ...
    graphHt = h;
    s = '';
    s += '<div style="position:relative; top: 70px; width:' + w + 'px; height:' + h + 'px; margin:auto; display:block; border: 2px solid lightblue; box-shadow: 0px 0px 9px 5px rgba(0,0,68,0.46);">';
    ...
    s += '</div>';
    s += '<div id="btns1" style="position:absolute; right:3px; top:3px;">';
    s += '<br>';
...
function poly2shapes() {
    ...
    for (i = 0; i < pts.length; i++) {
        var pt = new Pt(coords.toXVal(pts[i].x), coords.toYVal(pts[i].y));
        shapes.push(pt);
    }
}
...
function mouseMoveListener(evt) {
    ...
    if (dragScreenQ) {
        ...
        if (regQ) {
            poly.pts[dragNo].x = posX;
            poly.pts[dragNo].y = posY;
        }
    }
    doType();
}
...
function makeShapes() {
    ...
    console.log("makeShapes", this.modePts, shapes, poly.pts);
}
...
function drawPts() {
    ...
    if (drawShapeQ) poly.drawLines(g);
...
Poly.prototype.drawLines = function(g) {
    g.beginPath()
...
Poly.prototype.drawGuides = function(g, orig) {
...
Poly.prototype.isConcave = function() {
    for (var i = 0; i < this.pts.length; i++) {
        var pt = this.pts[i];
...
Poly.prototype.makeRegular = function() {
    var ptC = new Pt();
    ptC.setAvg(this.pts);
    var rad = 1
    var avgQ = false
    ...
    var sttAngle = Math.atan2(pts[0].y - ptC.y, pts[0].x - ptC.x);
    var dAngle = Math.PI * 2 / this.pts.length;
    for (i = 0; i < this.pts.length; i++) {
        var angle = sttAngle + i * dAngle;
        var pt = this.pts[i];
        pt.x = ptC.x + rad * Math.cos(angle);
        pt.y = ptC.y + rad * Math.sin(angle);
    }
};
